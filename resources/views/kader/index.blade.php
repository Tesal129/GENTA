...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        :root { --g-bg:#F7FBFF; --g-bg2:#EEF9F4; --g-green:#0E9E72; --g-green-lite:#E6F7F1; --g-green-border:rgba(14,158,114,.18); --g-blue:#1565C0; --g-blue-mid:#1976D2; --g-dark:#0A1628; --g-text:#1A2E3B; --g-text2:#3D5A6C; --g-muted:#7A9BB0; --g-border:rgba(21,101,192,.1); --g-white:#FFFFFF; --sidebar-w:240px; }
        ...
        .content { padding:28px 32px; flex:1; display:grid; grid-template-columns:1fr 340px; gap:24px; align-items:start; }
        .alert { padding:12px 16px; border-radius:10px; font-size:13px; font-weight:700; margin-bottom:20px; grid-column:1/-1; display:flex; align-items:center; gap:8px; }
        ...
        .card-header { padding:18px 24px; border-bottom:1px solid var(--g-border); }
        .card-title { font-size:15px; font-weight:900; color:var(--g-dark); }
        table { width:100%; border-collapse:collapse; }
        th { text-align:left; font-size:11px; font-weight:700; text-transform:uppercase; color:var(--g-muted); padding:12px 24px; background:var(--g-bg); border-bottom:1px solid var(--g-border); }
        td { padding:14px 24px; border-bottom:1px solid rgba(21,101,192,.05); font-size:13px; }
        .role-badge { padding:4px 10px; border-radius:100px; font-size:11px; font-weight:700; }
        .role-admin { background:rgba(21,101,192,.1); color:var(--g-blue); }
        .role-kader { background:var(--g-green-lite); color:var(--g-green); }
        .btn-icon { width:30px; height:30px; border-radius:7px; display:inline-flex; align-items:center; justify-content:center; border:none; cursor:pointer; background:rgba(239,68,68,.08); color:#DC2626; }
        .form-card { padding:24px; }
        .form-card h2 { font-size:15px; font-weight:900; color:var(--g-dark); margin-bottom:18px; }
        .form-group { margin-bottom:14px; }
        label { display:block; font-size:13px; font-weight:700; color:var(--g-text2); margin-bottom:6px; }
        input, select { width:100%; padding:10px 14px; border:1.5px solid var(--g-border); border-radius:8px; font-family:'Lato',sans-serif; font-size:14px; background:var(--g-bg); }
        .btn-submit { width:100%; margin-top:8px; padding:10px; background:var(--g-blue); color:#fff; border:none; border-radius:8px; font-weight:700; cursor:pointer; }
        .pagination-wrap { padding:16px 24px; border-top:1px solid var(--g-border); }
...
<div class="main">
    ...
    <div class="content">
        ...
        <div class="card">
            <div class="card-header"><div class="card-title">Daftar Kader ({{ $kaders->total() }})</div></div>
            <table>
                <thead>
                    <tr><th>Nama</th><th>Username</th><th>No HP</th><th>Role</th><th>Aksi</th></tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $kader->no_hp ?? '-' }}</td>
                        <td><span class="role-badge role-{{ $kader->role }}">{{ ucfirst($kader->role) }}</span></td>
                        <td>
            ...
            <div class="pagination-wrap">{{ $kaders->links() }}</div>
        </div>
